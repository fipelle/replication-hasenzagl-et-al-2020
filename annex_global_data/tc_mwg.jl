...
# ----------------------------------------------------------------------------------------------------------------------
     ...
     d   = zeros(n);
     Z   = [ones(n) [0; ones(n-1)] [zeros(n-2); ones(2)] [zeros(3); ones(n-3)] [zeros(5); ones(n-5)] [zeros(6); 1 ./ σʸ[end-3:end]]];
     Z1a = kron(Matrix(I, 6, 6), [1, 0, 1])';                                                                                     # idio C, idio C+, idio trend
     ...
     Z_ind[9:end, 3]     .= true; # Expect.    -> PC cycle, t-2
     Z_ind[5, 4]          = true; # Prices     -> GD cycle, t (BDI)
     Z_ind[6:end, [4,5]] .= true; # Prices     -> GD cycle, t and t-1

     # Z_plus_ind, Z_minus_ind and Z_bounded_ind (not used here)
     Z_plus_ind = zeros(size(Z)) .!= 0;
     Z_minus_ind = zeros(size(Z)) .!= 0;     
     Z_bounded_ind = zeros(size(Z)) .!= 0;     
     ...
     c              = zeros(size(Z)[2]);
     ind_trends     = [9; 12; 18]; # GDP, EMPL, GLOBAL IP
     ...
     T = cat(dims=[1,2], T_c_ext, [T_ct for i=1:7]..., [T_c for i=1:2]..., [T_ct for i=1:2]...);
     Q = cat(dims=[1,2], Q_c_ext, [T_ct for i=1:7]..., [T_c for i=1:2]..., [T_ct for i=1:2]...);
     ...
     P̄¹    = cat(dims=[1,2], zeros(3,3), [P̄_ct for i=1:7]..., [P̄_c for i=1:2]..., [P̄_ct for i=1:2]...);
     ...
     λ     = vcat([1; 0; 0], [λ_ct for i=1:7]..., [λ_c for i=1:2]..., [λ_ct for i=1:2]...);
     ρ     = copy(λ);
     λ_ind = λ .!= 0;
     ρ_ind = copy(λ_ind);
